...
import {getInvestmentFundHistory} from '../../store/actions';
...
const InvestmentWalletHistory = props => {
  const {loading, get_InvestmentWalletHistory, investment_wallet_history} =
    props;

  const [showMore, setShowMore] = useState(false);
  useEffect(() => {
    get_InvestmentWalletHistory();
  }, []);
...
const mapDispatchToProps = dispatch => ({
  get_InvestmentWalletHistory: () => dispatch(getInvestmentFundHistory()),
});

const mapStateToProps = state => {
  console.log('here is on investment fund history', state);
  return {
    status: state.wallet.status,
    loading: state.wallet.loading,
    investment_wallet_history: state.wallet.investmentfundhistory,
  };
};
