...
import {getTransferFundReport} from '../../store/actions';
...
const TransferFundReport = props => {
  const {loading, get_TransferFundReport, transfer_fund_report} = props;

  const [showMore, setShowMore] = useState(false);
  useEffect(() => {
    get_TransferFundReport();
  }, []);
...
const mapDispatchToProps = dispatch => ({
  get_TransferFundReport: () => dispatch(getTransferFundReport()),
});
...
const mapStateToProps = state => {
  ...
  return {
    ...
    transfer_fund_report: state.wallet.transferfundreport,
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(TransferFundReport);
